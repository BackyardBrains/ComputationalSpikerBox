...
inline void clickSpeaker(float membraneVoltage) {
  if (membraneVoltage > 0.0f) {
    ...
    CSB_BoardRGB.setPixelColor(0, CSB_BoardRGB.Color(255, 255, 255));
...
void updateHodgkinHuxleyET(){
 CSB_BoardRGB.clear();
  CSB_BoardRGB.setBrightness(50); 
  CSB_BoardRGB.setPixelColor(0, CSB_BoardRGB.Color(255, 150, 0));
  CSB_BoardRGB.show();
  I_Vm = I_Vm * 8;
...
void updateIzhikevich(){
  ...
  if(joystickMove > 25){
    ...
    swap = 0;
    selection++;
      if(selection ==6){ //Loop back around, selection should be 0, 1, 2, 3 or 4
        selection = 0;
      }
      if(selection == 2){
        selection = 3;
...
SKIN_NadaRGB.clear();
    ...
    CSB_BoardRGB.show();
    SKIN_NadaRGB.setPixelColor(0, SKIN_NadaRGB.Color(NeuronG[Neuronum],  NeuronR[Neuronum], NeuronB[Neuronum]));
    SKIN_NadaRGB.show();
