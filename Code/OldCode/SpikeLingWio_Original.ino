...
const int plotWidth = 320;  // width of plot
const int plotHeight = 240;
int plotX = 0;
float plotY[plotWidth];  // buffer for plot
...
int VmPotiScaling = 3;                                                           // the lower, the stronger the impact of the Vm poti.  Default = 2
...
void loop(void) {
  ...
  if (Array_DigiOutMode[NeuronBehaviour] == 2) {  // if in Noise Mode
    ...
    if (randNumber >= 50) {
      digitalWrite(DigitalIn1Pin, HIGH);
      Stim_State = 1;
    }
  }
  ...
  // add new V to end
  plotY[plotWidth - 1] = map(v, -70, 30, 0, plotHeight);
  // draw plot
  for (int i = 0; i < plotWidth - 1; i++) {
    tft.drawLine(i, plotHeight - plotY[i], i + 1, plotHeight - plotY[i + 1], TFT_WHITE);
  }

  // redraw background
  tft.fillRect(plotWidth - 1, 0, 1, plotHeight, TFT_BLACK);
